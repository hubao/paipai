...
////hash_map
#ifndef  WIN32
#define FTalk_hash_map        __gnu_cxx::hash_map
#else
#define FTalk_hash_map        stdext::hash_map
#endif
...
typedef uint64        WEIBOID;    //weibo id
typedef uint64        USERID;        //user id
typedef uint64        ACCOUNTID;        //user id
typedef uint64        TEXTMSGID;    //text msg id;
typedef uint64        CALLID;        //
typedef uint32        GROUPID;
typedef uint32        SERVERID;    //服务器ID
typedef uint32        TOKENID;
typedef int64        ANNOUNCEID;    //
typedef int64        TRANSACTIONID;    //
typedef int64        TRADENOID;    //
typedef int32        ROOMID;    //
typedef int64        T_WEALTH;    //
typedef int64        T_CASH;    //
typedef int32       CHANNELID;
typedef int32       AREAID;
typedef int32       GOODSID;
...
#define FTalk_ASSERT(f) if(!(f)){LOG_TRACE(1,false,__FUNCTION__," !Assert"<<__LINE__)};assert(f);

#ifdef WIN32
#define CHECKMEMORY  FTalk_ASSERT(_CrtCheckMemory())
#endif
#endif //FTalk_ASSERT
...
//是匿名用户ID
#define isGuestID(aiUserID) ( ((aiUserID>>48)== 0x66))
...
//////////////////////////////////////////////////////////////////////////
//FTalk namespace
#define FTalk_BEGIN_NAMESPACE namespace FTalk {
#define FTalk_END_NAMESPACE   }
...
// 构造失败的返回值
#define MAKE_ERROR_RESULT(aiErrorCode)  ((uint32_t)aiErrorCode & 0x7FFFFFFF)

// 检查结果是成功还是失败
#define CHECK_RESULT(adwResult)          ((uint32_t)adwResult == 0x80000000)
// 从结果中获得错误码
#define GET_ERROR_CODE(adwResult)        ((uint32_t)adwResult & 0x7FFFFFFF)
